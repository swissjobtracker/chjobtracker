...
# TODO: Something goes awry with time zones. Since Zurich is ahead of UTC
# nothing gets lost though.
last_update <- DBI::dbGetQuery(
  con, "select max(t) from x28.event_log where event = 'x28_update_db'"
)$max

if (is.na(last_update)) {
  message(
    "This seems to be the first run, asking for 2 days of stuff (starting ",
    default_update,
    ")."
  )
...
if (last_update < default_update) {
  warning(
    ...
    ") is more than 2 days old (", default_update,
    ")!\nDoing what I can..."
  )
  last_update <- default_update
}
